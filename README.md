...
## Render へのデプロイ手順
Render の Web Service として Node 環境で実行できます。リポジトリルートの `render.yaml` に必要な設定が含まれているため、Render へリポジトリを接続するだけでデプロイが行えます。

1. Render で新しい Web Service を作成し、リポジトリを接続します。
2. ビルドコマンドとスタートコマンドは `render.yaml` に記載済み（`npm install` / `npm start`）です。
3. 環境変数は Render が `PORT` を自動設定するため追加は不要です。`HOST` は `0.0.0.0` に設定済みです。
4. デプロイ後、Render のダッシュボードの URL にアクセスして動作を確認します。ヘルスチェックは `/health` で確認できます。
